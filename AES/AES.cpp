...
AES_ERROR AES_Encrypt(vector<unsigned char> key, vector<unsigned char> Buf, vector<unsigned char>& BufEncrypt,AES_MODE mode,AES_TYPE type)
{
    ...
    else if(key.size()== 4*6){
        Nr = 12;
        Nk = 6;
    }
    else if (key.size()== 4*8){
        Nr = 14;
        Nk = 8;
    }
    else {
...
AES_ERROR AES_Decrypt(vector<unsigned char> key, vector<unsigned char> Buf, vector<unsigned char>& BufDecrypt,AES_MODE mode)
{
    uint8_t Nk, Nr;
